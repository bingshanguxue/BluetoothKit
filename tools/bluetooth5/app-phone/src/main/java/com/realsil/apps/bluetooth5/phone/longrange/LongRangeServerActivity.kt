...
import kotlinx.android.synthetic.main.phone_activity_longrange_transmitter.*
import java.util.*
...
class LongRangeServerActivity : GattServerActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.phone_activity_longrange_transmitter)
        ...
        logView.configure(LogView.LogConfigure.Builder()
        ...
        if (characteristic != null) {
            if (LongRangeProfile.RX_CHARACTERISTIC == characteristic.uuid) {
                device?.let {
                    ...
                    runOnUiThread {
                        logView.i(
                            ">> ${device.address} - ${characteristic.uuid} ${
                                DataConverter.bytes2Hex(
                                    value
                                )
                            }"
                        )
...
//        syncExecutor?.shutdown()
    ...
    private var syncExecutor: ThreadPoolExecutor? = null
